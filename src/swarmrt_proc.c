 ...
 *
 * Reduction counting, preemptive scheduling, copying message passing,
 * per-process heaps with generational GC, work-stealing schedulers.
 *
 * otonomy.ai
 */
...
/* === Global State === */
static sw_proc_swarm_t *g_swarms[16];
static int g_num_swarms = 0;
static pthread_mutex_t g_swarms_lock = PTHREAD_MUTEX_INITIALIZER;

/* Thread-local current scheduler */
__thread sw_proc_scheduler_t *tls_scheduler = NULL;
__thread sw_proc_process_t *tls_current = NULL;

/* === Forward Declarations === */
static void *scheduler_thread(void *arg);
static void run_process(sw_proc_scheduler_t *sched, sw_proc_process_t *proc);
static void sw_proc_add_to_runq(sw_proc_runq_t *rq, sw_proc_process_t *proc);
static sw_proc_process_t *sw_proc_pick_next(sw_proc_scheduler_t *sched);

/* ============================================================================
 * HEAP MANAGEMENT (per-process heaps)
 * ============================================================================ */

static int heap_init(sw_proc_heap_t *heap, size_t initial_words) {
    size_t bytes = initial_words * sizeof(sw_proc_term_t);
    heap->start = (sw_proc_term_t *)malloc(bytes);
...
static void heap_free(sw_proc_heap_t *heap) {
    if (heap->start) {
        free(heap->start);
        heap->start = NULL;
    }
...
sw_proc_term_t *sw_proc_heap_alloc(sw_proc_process_t *proc, size_t words) {
    ...
    if (heap->top + words > heap->end) {
        ...
        if (heap->top + words > heap->end) {
            ...
            sw_proc_term_t *new_start = realloc(heap->start, new_size * sizeof(sw_proc_term_t));
            if (!new_start) {
                fprintf(stderr, "[SwarmRT] Heap allocation failed!\n");
                exit(1);
            }
    ...
    sw_proc_term_t *result = heap->top;
    heap->top += words;
    return result;
}

/* ============================================================================
 * TERM COPYING (process isolation - copy to receiver's heap)
 * ============================================================================ */

static size_t term_size(sw_proc_term_t *term) {
    if (!term) return 0;
    
    switch (term->type) {
        case SW_PROC_ATOM:
            return 1; /* Header */
        case SW_PROC_INTEGER:
            return 1;
        case SW_PROC_FLOAT:
        ...
        case SW_PROC_BINARY:
            return 1 + (term->val.binary.size + sizeof(sw_proc_term_t) - 1) / sizeof(sw_proc_term_t);
        default:
            return 1;
    }
}

static sw_proc_term_t *copy_term_recursive(sw_proc_process_t *to, sw_proc_term_t *term) {
    if (!term) return NULL;
    
    sw_proc_term_t *copy = sw_proc_heap_alloc(to, term_size(term));
    copy->type = term->type;
    ...
    switch (term->type) {
        ...
        case SW_PROC_TUPLE: {
            ...
            copy->val.tuple.items = (sw_proc_term_t **)(copy + 1);
...
void sw_proc_gc(sw_proc_process_t *proc) {
    ...
    /* TODO: full generational GC with minor/major collections */
    
    proc->state = SW_PROC_PROC_GARBING;
    proc->heap.gen_gcs++;
    
    /* For now, just expand heap - proper GC is complex */
    /* Full implementation would trace from roots (stack, regs, mailbox) */
    
    proc->state = SW_PROC_PROC_RUNNABLE;
}
...
void sw_proc_send(sw_proc_process_t *to, sw_proc_term_t *msg) {
    ...
    sw_proc_msg_t *m = (sw_proc_msg_t *)malloc(sizeof(sw_proc_msg_t));
    
    /* Copy message to receiver's heap (process isolation) */
    m->payload = sw_proc_copy_term(to, msg);
    m->from = tls_current;
    ...
    if (to->state == SW_PROC_PROC_WAITING) {
        to->state = SW_PROC_PROC_RUNNABLE;
        if (to->scheduler) {
            sw_proc_add_to_runq(&to->scheduler->runq, to);
        }
    }
...
sw_proc_term_t *sw_proc_receive(uint64_t timeout_ms) {
    sw_proc_process_t *proc = tls_current;
    if (!proc) return NULL;
    ...
    while (1) {
        ...
        if (proc->mailbox.first) {
            sw_proc_msg_t *m = proc->mailbox.first;
            ...
            sw_proc_term_t *payload = m->payload;
            free(m);
            return payload;
        }
...
static sw_proc_process_t *sw_proc_pick_next(sw_proc_scheduler_t *sched) {
    sw_proc_runq_t *rq = &sched->runq;
    
    pthread_mutex_lock(&rq->lock);
    
    /* Pick from highest priority first */
    for (int prio = 0; prio < 4; prio++) {
        sw_proc_process_t *p = rq->prio_heads[prio];
        if (p) {
            /* Remove from queue */
...
sw_proc_process_t *sw_proc_schedule(sw_proc_scheduler_t *sched, 
    ...
    if (proc) {
        ...
        if (proc->state == SW_PROC_PROC_RUNNING) {
            /* Still runnable, add back to queue */
            sw_proc_add_to_runq(&sched->runq, proc);
        }
    }
    
    /* Find next process to run */
    sw_proc_process_t *next = sw_proc_pick_next(sched);
    
    /* Try work stealing if local queue empty */
    if (!next && sched->swarm) {
        next = sw_proc_steal_work(sched);
    }
    ...
    /* Give it a fresh reduction budget */
    next->fcalls = SWARM_PROC_CONTEXT_REDS;
    next->state = SW_PROC_PROC_RUNNING;
...
sw_proc_process_t *sw_proc_steal_work(sw_proc_scheduler_t *sched) {
    ...
    for (uint32_t i = 0; i < sched->swarm->num_schedulers; i++) {
        ...
        sw_proc_scheduler_t *victim = sched->swarm->schedulers[i];
        if (!victim) continue;  /* Not created yet */
        sw_proc_runq_t *vrq = &victim->runq;
        ...
        for (int prio = 3; prio >= 0; prio--) {
            if (vrq->prio_counts[prio] > 1) {
                ...
                sw_proc_process_t *stolen = vrq->prio_tails[prio];
...
static void run_process(sw_proc_scheduler_t *sched, sw_proc_process_t *proc) {
    ...
    /* Process completed */
    proc->state = SW_PROC_PROC_EXITING;
...
static void *scheduler_thread(void *arg) {
    sw_proc_scheduler_t *sched = (sw_proc_scheduler_t *)arg;
    ...
    while (sched->active && sched->swarm && sched->swarm->running) {
        ...
        sw_proc_process_t *proc = sw_proc_pick_next(sched);
        
        /* Try work stealing if empty */
        if (!proc) {
            proc = sw_proc_steal_work(sched);
        }
        
        if (proc) {
            /* Give process a time slice */
            proc->fcalls = SWARM_PROC_CONTEXT_REDS;
            proc->state = SW_PROC_PROC_RUNNING;
...
int swarm_proc_init(const char *name, uint32_t num_schedulers) {
    ...
    sw_proc_swarm_t *swarm = (sw_proc_swarm_t *)calloc(1, sizeof(sw_proc_swarm_t));
    ...
    swarm->process_table = (sw_proc_process_t **)calloc(SWARM_PROC_MAX_PROCESSES,
                                                         sizeof(sw_proc_process_t *));
    
    /* Create schedulers */
    for (uint32_t i = 0; i < num_schedulers; i++) {
        sw_proc_scheduler_t *sched = (sw_proc_scheduler_t *)calloc(1, sizeof(sw_proc_scheduler_t));
        sched->id = i;
        sched->swarm = swarm;
...
void swarm_proc_shutdown(int swarm_id) {
    ...
    sw_proc_swarm_t *swarm = g_swarms[swarm_id];
    swarm->running = 0;
...
sw_proc_process_t *sw_proc_spawn(void (*func)(void*), void *arg) {
    return sw_proc_spawn_on(0, 0, func, arg);
}
...
sw_proc_process_t *sw_proc_spawn_on(int swarm_id, uint32_t sched_id, 
    ...
    /* Allocate process */
    sw_proc_process_t *p = (sw_proc_process_t *)calloc(1, sizeof(sw_proc_process_t));
    ...
    p->arg = arg;
    p->priority = 2; /* NORMAL */
    p->fcalls = SWARM_PROC_CONTEXT_REDS;

    /* Initialize heap */
    if (heap_init(&p->heap, SWARM_PROC_HEAP_MIN_SIZE) != 0) {
 ...
 * Full preemption uses reduction counting.
 */
...
void sw_proc_wait(sw_proc_process_t *proc) {
    proc->state = SW_PROC_PROC_WAITING;
    /* Cooperative mode - we need to poll */
    /* Full impl would swapcontext to scheduler */
    usleep(1000); /* 1ms sleep to prevent busy loop */
}

void sw_proc_wake(sw_proc_process_t *proc) {
    if (proc->state == SW_PROC_PROC_WAITING) {
        proc->state = SW_PROC_PROC_RUNNABLE;
        sw_proc_add_to_runq(&proc->scheduler->runq, proc);
    }
}
...
sw_proc_term_t *sw_proc_mk_int(sw_proc_process_t *proc, int64_t val) {
    sw_proc_term_t *t = sw_proc_heap_alloc(proc, 1);
    t->type = SW_PROC_INTEGER;
    t->val.i = val;
    return t;
}
...
sw_proc_term_t *sw_proc_mk_atom(const char *name) {
    ...
    static struct { char *name; sw_proc_term_t *term; } atoms[1000];
    ...
    /* Create new atom */
    sw_proc_term_t *t = (sw_proc_term_t *)malloc(sizeof(sw_proc_term_t));
    t->type = SW_PROC_ATOM;
...
void swarm_proc_stats(int swarm_id) {
    ...
    for (uint32_t i = 0; i < swarm->num_schedulers; i++) {
        sw_proc_scheduler_t *sched = swarm->schedulers[i];
