...
/* Batteries the program imports (see g_batteries in swarmrt_lang.c): set by
 * the codegen entry points before emit_preamble, which turns each into a
 * `#define SW_BATTERY_<X> 1` ahead of the builtins header. */
static int g_cg_battery_on[16];

static void cg_set_batteries(node_t **mods, int nmods) {
    for (int b = 0; b < sw_battery_count() && b < 16; b++) {
        g_cg_battery_on[b] = 0;
        for (int m = 0; m < nmods; m++)
            if (sw_module_has_battery(mods[m], sw_battery_module_at(b))) { g_cg_battery_on[b] = 1; break; }
    }
}
...
static void emit_preamble(cg_ctx_t *ctx) {
    ...
    for (int b = 0; b < sw_battery_count() && b < 16; b++)
        if (g_cg_battery_on[b]) fprintf(f, "#define %s 1\n", sw_battery_macro_at(b));
...
int sw_codegen_multi(void **modules, int nmodules, int main_idx, FILE *out) {
    ...
    cg_set_batteries((node_t **)modules, nmodules);
