...
- **Memory is bounded under fixed concurrency (Ownership v2), with two remaining gaps.** A process's own working set frees on exit; sent messages and spawn args are adopted into receiver/child ownership and reclaimed; long-lived tail loops are bounded by a scoped turn-checkpoint (`make gc-slope` proves spawn / message / 100k-turn slopes stay flat). **Remaining:** (1) **ETS** entries are still global-heap and reclaimed only on table destroy — a high-churn ETS replace/delete loop grows until then; (2) the **interpreter** has no arena (its values free at OS exit — fine for short-lived `swc run`/REPL). Values nested deeper than 256 levels are truncated on cross-process copy.
