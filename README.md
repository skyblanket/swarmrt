...
| **Avoiding language overhead** | One binary, no VM, no GC pauses, <10ms startup. (Memory model — GC v1: each sw process has a **value arena** freed wholesale when that process exits, so a worker's *own* working set is reclaimed (50k workers building+dropping local garbage held ~490 MB vs ~4.5 GB without the arena). **Honest caveat:** values that *escape* a process — sent messages, spawn arguments, ETS entries — are deep-copied to the global heap and are **not yet reclaimed**, so a fixed-concurrency workload passing *large* escaped values (big prompts, tool outputs) still grows with cumulative jobs; and a single long-lived loop accumulates its per-turn garbage until it exits. Lifecycle-owned regions for escaped values + per-turn loop reset are **Ownership v2**, in progress. Cross-process messages are deep-copied, BEAM-style.) The binary statically links libswarmrt and dynamically links the four system libs above — no runtime install or VM image. |
...
| **Process** | 2KB arena-allocated PCB + 128KB stack. Lock-free MPSC mailbox. Per-process **value arena**, reclaimed wholesale at process exit; cross-process messages deep-copied (no shared heap). Escaped values (messages/spawn args/ETS) currently copy to the global heap and aren't yet lifecycle-reclaimed; owned regions + mid-life loop reset are **Ownership v2** (in progress). |
...
- **GC v1 reclaims a process's *own* working set on exit, but not escaped values or long-lived loops yet.** A worker's locally-built garbage is freed when the worker exits — but values that *escape* (sent messages, spawn arguments, ETS entries) are deep-copied to the global heap and not yet lifecycle-reclaimed, so a fixed-concurrency workload passing *large* escaped values still grows with cumulative jobs; and a single forever-loop accumulates its per-turn garbage until it exits. Owned regions for escaped values + per-turn loop reset are **Ownership v2** (in progress). Values nested deeper than 256 levels are truncated on cross-process copy.
