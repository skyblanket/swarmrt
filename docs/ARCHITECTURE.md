...
- **GC / Ownership v2 (`swarmrt_varena.c`)** — per-process value arena, freed when the process exits. Escaped values get a lifecycle owner (no shared heap, BEAM-style): a sent message is deep-copied into a *region* the receiver **adopts** (O(1) chunk splice) into its own arena on match; spawn arguments are adopted into the child's arena; a long-lived tail-recursive loop is bounded by a **scoped turn-checkpoint** — it records an arena "floor" at function entry and, once it accumulates past a threshold above the floor, copies the recursion args into a temp region, rewinds the arena to the floor (reclaiming this turn's garbage but never the caller's values below it), and splices the args back. So fixed-concurrency spawn/message workloads and 100k-turn loops hold flat peak RSS (`make gc-slope`). Supervisor/timer closures are copied to the global heap; ETS values are global-heap until table destroy. The older `swarmrt_gc.c` scaffolding is unused.
