...
2. **State is the recursion arg.** `agent(name, new_state)` is how you "update" state. The compiler turns self-tail-calls into `goto`, so the **stack** doesn't grow across iterations. (Heap value memory: each sw process has a **value arena** that's freed when that process exits, so spawning short-lived workers DOES reclaim — peak memory tracks *concurrently-live* processes, not total spawns. The remaining gap: a single long-lived process that loops forever still accumulates its own per-iteration garbage — bump-allocation can't free mid-life *within* a living process — so prefer a worker-per-job shape over one monolithic forever-loop; mid-life reclaim is the next GC step. Values sent between processes are deep-copied, BEAM-style.)
