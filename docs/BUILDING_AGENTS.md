...
2. **State is the recursion arg.** `agent(name, new_state)` is how you "update" state. The compiler turns self-tail-calls into `goto`, so the **stack** doesn't grow across iterations. (Heap value memory is a different story: there's no tracing GC yet, and values are reclaimed only when the **OS process** exits — not when an sw process does, so spawning short-lived workers doesn't reclaim either. A bounded job that exits is fine; a genuinely long-running, high-throughput loop accumulates memory and needs a periodic binary restart until the GC lands — the top roadmap item. Lighter per-iteration allocation slows the climb but doesn't stop it.)
