...
> - **Standard library**: `import Std` (list/map/string helpers + `Std.task_stream(list, fn, %{max_concurrency, timeout_ms, on_timeout})` bounded parallel fan-out returning tagged `{'ok',v}`/`{'error',_}` in input order, and `Std.json_expect(str, %{required:[...]})` decode-and-validate), `import Agent` (the canonical LLM loop: `Agent.new(%{url,key,model,system,tools})` then `Agent.ask(a, msg) -> {'ok',reply}|{'error',_}`, plus `Agent.ask_json` for schema-validated output), `import Mcp` (MCP client+server), `import Embed` + `import Vec` (embeddings + vector store), `import Prompt` (`{{var}}` templates), `import Cron` (`every(ms, fn)`, `at("HH:MM", fn)`), `import Telemetry` (event emit + sinks). All auto-resolve from `<swarmrt>/lib/`.
